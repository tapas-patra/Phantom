...
namespace SecureOverlay.Services
{
    public class OpenRouterService : IAIService
    {
        ...
        {
            if (!IsConfigured())
                return "Error: OpenRouter API key or model not configured. Go to Settings.";
        ...
        private HttpRequestMessage CreateRequest(object request)
        {
            var requestMessage = new HttpRequestMessage(HttpMethod.Post, ChatCompletionsUrl)
            {
                Content = new StringContent(JsonConvert.SerializeObject(request), Encoding.UTF8, "application/json")
            };
            requestMessage.Headers.Authorization = new AuthenticationHeaderValue("Bearer", _apiKey);
            requestMessage.Headers.TryAddWithoutValidation("HTTP-Referer", "https://phantom.app");
            requestMessage.Headers.TryAddWithoutValidation("X-Title", "Phantom");
