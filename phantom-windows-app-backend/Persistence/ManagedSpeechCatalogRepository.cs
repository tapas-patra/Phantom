...
public sealed class ManagedSpeechCatalogRepository
{
    private readonly PostgresBackendStore _store;

    public ManagedSpeechCatalogRepository(PostgresBackendStore store) => _store = store;
...
FROM managed_speech_provider_catalog
...
LIMIT 1;";
    ...
    public void Save(ManagedProviderCatalogRecord record)
    {
        ...
        using var command = connection.CreateCommand();
        command.CommandText = @"
INSERT INTO managed_speech_provider_catalog (provider_id, label, models_json, refreshed_at_utc)
VALUES (@providerId, @label, CAST(@modelsJson AS jsonb), @refreshedAtUtc)
ON CONFLICT (provider_id) DO UPDATE SET
    ...
    private static ManagedProviderCatalogRecord Map(NpgsqlDataReader reader) => new()
    {
