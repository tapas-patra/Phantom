...
enum CopilotPrompt {
    ...
    static func firstCall(
        ...
        if mode == .interview { prompt += " \(interviewContracts) \(style == .desi ? desiStyle : standardStyle)" }
    ...
    private static let standardStyle = "Delivery style is Standard — polished, concise professional spoken English. Use neutral transitions, complete sentences, and restrained Markdown emphasis. Avoid casual openers and colloquial filler."
    private static let desiStyle = "Delivery style is Desi — sound like a confident Indian professional speaking naturally, not like a formal written answer. Use short conversational sentences, concrete explanations, and varied spoken bridges such as 'So what I did was', 'The main thing is', or 'In practice' only when they fit; never repeat a stock opener. Use selective bold emphasis on 2–4 phrases that help the candidate scan the answer. For behavioral answers, use four short spoken paragraphs covering situation, action, result, and learning without adding headings. For technical answers, start simply, use one accurate relatable analogy when useful, then give the practical tradeoff or example. Do not imitate an accent or stereotype, use broken grammar, force slang or Hinglish, repeat filler, invent cultural or company examples, or weaken technical accuracy. Code-switch only when the user's current language naturally supports it. For SQL versus NoSQL, never claim SQL only scales up or NoSQL only scales out; both can scale horizontally depending on the database."
}
